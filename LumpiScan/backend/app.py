...
def run_inference(img_bytes):
    """
    Handles both model output shapes automatically:
      - Sigmoid binary     : output shape (1,) — single neuron, value in [0, 1]
      - Softmax categorical: output shape (2,) — two neurons, sum to 1
    Returns (prediction_idx, confidence)
    """
    processed = preprocess_image(img_bytes)
    preds = model.predict(processed)[0]  # shape: (1,) or (2,)

    if len(preds) == 1:
        # ── Sigmoid / binary crossentropy ─────────────────────────────────────
        # preds[0] = P(LSD). If >= 0.5 → infected, else → healthy.
        sigmoid_val = float(preds[0])
        prediction_idx = 1 if sigmoid_val >= 0.5 else 0
        # Confidence = distance from boundary, always in [0.5, 1.0]
        confidence = sigmoid_val if prediction_idx == 1 else (1.0 - sigmoid_val)
    else:
...
def is_cow(img_bytes, threshold=0.5):
    """
    Runs the cow-validator model on raw image bytes.
    Returns True if the image contains a cow, False otherwise.
    Assumes sigmoid output: value >= threshold → is a cow.
    """
    processed = preprocess_image(img_bytes)
    preds = cow_model.predict(processed)[0]
    score = float(preds[0]) if len(preds) == 1 else float(preds[1])
...
def health():
    return jsonify({
        ...
        "cow_model_loaded": cow_model is not None,   # ← add
        "cow_model_error": cow_model_error,  
        "cow_model_path": os.path.abspath(COW_MODEL_PATH),         # ← add
    })
...
def predict():
    ...
    # ── Step 1: Validate that the image contains a cow ────────────────────────
    if cow_model is not None:
        if not is_cow(img_bytes):
            return jsonify({
                "error": "Invalid image",
                "message": "The uploaded image does not appear to contain a cow. Please upload a clear photo of the animal.",
                "is_valid_cow": False
            }), 422   # 422 Unprocessable Entity — valid image, wrong content
    # if cow_model failed to load, we skip validation and proceed anyway
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # model = tf.keras.models.load_model("lsd_final.keras", compile=False)
    ...
    if model is None:
        ...
        prediction_idx, confidence = run_inference(img_bytes)
